...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const missingLeadTargets = [...requiredLeadTargets].filter((target) => !mappedTargets.has(target));
    const allMissingTargets = [...new Set(missingLeadTargets)];

    if (allMissingTargets.length > 0) {
      return jsonResponse({
        error: 'Required fields are not mapped. Lead name and lead email are mandatory.',
        missing_required_targets: allMissingTargets,
      }, 400);
    }
