...
export async function analyzeImportColumns(params: {
  ...
  const enforcedLeadMissingTargets = [...REQUIRED_LEAD_TARGETS].filter((target) => !mappedTargets.has(target));
  ...
  return {
    ...
    suggestions,
    requiredMissingTargets: [...new Set([...combinedRequiredMissingTargets, ...enforcedLeadMissingTargets])],
  };
}
