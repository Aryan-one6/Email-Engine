...
export async function createRecordForWorkspace(serviceClient: EdgeClient, userId: string, payload: SaveRecordPayload) {
  ...
  const requestedPipelineId = normalizeNullableString(core.pipeline_id);
  const requestedStageId = normalizeNullableString(core.stage_id);
  ...
  if (stageId && !pipelineId) {
    throw new Error('Pipeline is required when stage is provided.');
  }
  ...
  const requestedStatus = normalizeRecordStatus(core.status) ?? 'new';
...
export async function updateRecordForWorkspace(
  ...
  const requestedPipelineId =
    core.pipeline_id === undefined ? existing.record.pipeline_id : normalizeNullableString(core.pipeline_id);
  const requestedStageId =
    core.stage_id === undefined ? existing.record.stage_id : normalizeNullableString(core.stage_id);
