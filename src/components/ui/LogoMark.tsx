...
export function LogoMark({ className, showSubtitle = true, theme = 'light' }: LogoMarkProps) {
  ...
  return (
    <div className={cn('flex items-center gap-2.5', className)}>
      ...
      <div>
        <div className={cn('font-display text-[15px] font-semibold leading-tight', isDark ? 'text-white' : 'text-slate-900')}>
          Email Engine
        </div>
        {showSubtitle ? (
          <div className={cn('text-[10px] uppercase tracking-widest', isDark ? 'text-white/75' : 'text-slate-400')}>
            Marketing Platform
          </div>
        ) : null}
      </div>
