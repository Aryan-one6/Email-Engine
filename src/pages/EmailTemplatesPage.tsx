...
const RECORD_STATUSES = ['new', 'email_sent', 'mobile_contacted', 'replied', 'interested', 'not_interested'];
...
function applySampleVariables(value: string, variableValues: Record<string, string>) {
  return value.replace(/{{\s*([a-zA-Z0-9_]+)\s*}}/g, (_, token: string) => variableValues[token] ?? `{{${token}}}`);
}
...
function TemplateEditorModal({
  ...
  const sampleVariables = useMemo(
    () => ({
      ...
      workspace_name: workspaceName || SAMPLE_VARIABLE_VALUES.workspace_name,
      sender_name:
        (typeof themeOverrides.sender_display_name_default === 'string' && themeOverrides.sender_display_name_default.trim()) ||
        SAMPLE_VARIABLE_VALUES.sender_name,
      sender_email:
        (typeof themeOverrides.sender_email_default === 'string' && themeOverrides.sender_email_default.trim()) ||
        SAMPLE_VARIABLE_VALUES.sender_email,
    }),
    [themeOverrides.sender_display_name_default, themeOverrides.sender_email_default, workspaceName],
  );

  const preview = useMemo(() => deriveTemplateOutputs(layout, themeOverrides), [layout, themeOverrides]);
  const previewSubject = useMemo(() => applySampleVariables(subject, sampleVariables), [sampleVariables, subject]);
  const previewHtml = useMemo(() => applySampleVariables(preview.preview_html, sampleVariables), [preview.preview_html, sampleVariables]);
