...
export function CrmWorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  async function ensureConfig() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unable to load records workspace config.';
  ...
  async function refreshConfig() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unable to refresh records workspace config.';
      setConfigError(message);
      return config;
    } finally {
      setConfigLoading(false);
